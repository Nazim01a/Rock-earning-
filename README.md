...
// Plans
let plans=[
{name:'Basic',amount:200,daily:30,days:20,limited:true,sold:false},
{name:'Starter',amount:500,daily:75,days:25,limited:true,sold:false},
{name:'Pro',amount:1000,daily:180,days:30,limited:true,sold:false},
{name:'Advanced',amount:1500,daily:250,days:35,limited:true,sold:false},
{name:'Silver',amount:2000,daily:350,days:40,limited:false},
{name:'Gold',amount:3000,daily:550,days:45,limited:false},
{name:'Platinum',amount:5000,daily:950,days:50,limited:false},
{name:'Diamond',amount:7000,daily:1350,days:55,limited:false},
{name:'Elite X',amount:10000,daily:2200,days:60,limited:false,coming:true},
{name:'Mega Booster',amount:15000,daily:3500,days:65,limited:false,coming:true},
{name:'Ultra Pro',amount:20000,daily:4800,days:70,limited:false,coming:true},
{name:'Crypto Miner',amount:30000,daily:6500,days:75,limited:false,coming:true},
{name:'Titanium Plan',amount:40000,daily:8500,days:80,limited:false,coming:true},
{name:'Ultimate',amount:50000,daily:12000,days:85,limited:false,coming:true},
{name:'Infinity',amount:100000,daily:25000,days:90,limited:false,coming:true}
];
...
if(p.coming){return `<div class="plan-card"><b>${p.name}</b> - ${p.amount} PKR - Daily: ${p.daily} PKR - ${p.days} Days<br><button class="btn mt-2" disabled>Coming Soon</button></div>`;}
let btnText=p.sold?"Sold":"Buy Now";
let btnDisable=p.sold?"disabled":""; 
return `<div class="plan-card"><b>${p.name}</b> - ${p.amount} PKR - Daily: ${p.daily} PKR - ${p.days} Days<br><button class="btn mt-2" onclick="buyPlan(${i})" ${btnDisable}>${btnText}</button></div>`;
}).join('');
}
else if(type==='deposit'){content.innerHTML="<h2>Deposit</h2><p>Deposit details...</p>";}
else if(type==='withdraw'){content.innerHTML="<h2>Withdraw</h2><p>Withdraw details...</p>";}
else if(type==='profit'){content.innerHTML=`<h2>Profit</h2><p>Total profit earned: ${currentUser.profit} PKR</p>`;}
else if(type==='history'){content.innerHTML="<h2>History</h2><p>Activity history...</p>";}
...
// Buy plan
function buyPlan(idx){
let p=plans[idx];
if(p.sold){showNotif('This plan is sold!');return;}
p.sold=true;
currentUser.plans.push({...p,ts:Date.now(),profitAdded:false});
currentUser.balance+=p.amount;
localStorage.setItem('reCurrent',JSON.stringify(currentUser));
showNotif(`${p.name} plan bought! Profit will add every 24h`);
openSection('plans');

// Reset sold status after 24h for limited plans
if(p.limited){
setTimeout(()=>{p.sold=false;openSection('plans');},24*60*60*1000);
}
}
...
if(!p.profitAdded && now-p.ts>=24*60*60*1000){
