...
.sidebar{position:fixed;top:0;left:0;width:80px;height:100vh;background:rgba(20,20,20,0.95);display:flex;flex-direction:column;align-items:center;padding-top:20px;gap:20px;z-index:10;display:none;}
.icon-btn{display:flex;flex-direction:column;align-items:center;padding:12px;border-radius:15px;cursor:pointer;transition:0.3s;background:#111;color:#ccc;box-shadow:0 0 5px #000;width:60px;}
.icon-btn:hover{transform:scale(1.2);box-shadow:0 0 15px #0ff,0 0 25px #0ff;}
.icon-name{margin-top:6px;font-size:10px;color:#0ff;text-align:center;text-shadow:0 0 3px #000;}
...
<div class="icon-btn" onclick="openSection('plans')"><i class="fa fa-gem fa-2x"></i><span class="icon-name">Plans</span></div>
<div class="icon-btn" onclick="openSection('deposit')"><i class="fa fa-money-bill fa-2x"></i><span class="icon-name">Deposit</span></div>
<div class="icon-btn" onclick="openSection('withdraw')"><i class="fa fa-hand-holding-dollar fa-2x"></i><span class="icon-name">Withdraw</span></div>
<div class="icon-btn" onclick="openSection('profit')"><i class="fa fa-coins fa-2x"></i><span class="icon-name">Profit</span></div>
<div class="icon-btn" onclick="openSection('history')"><i class="fa fa-clock fa-2x"></i><span class="icon-name">History</span></div>
<div class="icon-btn" onclick="openSection('support')"><i class="fa fa-headset fa-2x"></i><span class="icon-name">Support</span></div>
<div class="icon-btn" onclick="openSection('referral')"><i class="fa fa-share-alt fa-2x"></i><span class="icon-name">Referral</span></div>
<div class="icon-btn" onclick="openSection('share')"><i class="fa fa-share fa-2x"></i><span class="icon-name">Share</span></div>
</div>
...
<p style="margin-top:10px;color:#0ff;">Company: Rock Earn Ltd | 25 Main St, California, USA | Founded: 2025 | Secure & Premium Trading Platform</p>
</div>
...
<!-- Auth Box -->
<div id="authBox" class="card" style="margin-left:100px;margin-top:20px;width:320px;">
...
function signupUser() {
    ...
    let u = {name: n, email: e, pass: p, plans: [], balance: 0, profit: 0};
    users.push(u);
...
let plans = [
    ...
    {name:'Elite X',amount:10000,daily:2200,days:5,coming:true},
    {name:'Mega Booster',amount:15000,daily:3500,days:6,coming:true},
    {name:'Ultra Pro',amount:20000,daily:4800,days:6,coming:true},
    {name:'Crypto Miner',amount:30000,daily:6500,days:7,coming:true},
    {name:'Titanium Plan',amount:40000,daily:8500,days:7,coming:true},
    {name:'Ultimate',amount:50000,daily:12000,days:8,coming:true},
    {name:'Infinity',amount:100000,daily:25000,days:10,coming:true}
];
...
function openSection(type) {
    ...
    if(type === 'plans') {
        content.innerHTML = '<h2>Available Plans</h2>' + plans.map(p => {
            if(p.coming){
                return `<div class="plan-card"><b>${p.name}</b> - ${p.amount} PKR - Daily: ${p.daily} PKR - <span>${p.days} Days</span><br><button class="btn mt-2" disabled>Coming Soon</button></div>`;
            }
            return `<div class="plan-card"><b>${p.name}</b> - ${p.amount} PKR - Daily: ${p.daily} PKR - <span>${p.days} Days</span><br><button class="btn mt-2" onclick="openDeposit(${p.amount},'${p.name}',${p.daily},${p.days})">Buy Now</button></div>`;
        }).join('');
    }
        ...
        let hist = currentUser.history || [];
        content.innerHTML = `<h2>Activity History</h2>` + hist.map(h => `<p>• ${h}</p>`).join('');
    }
...
function confirmDeposit(amount,name,daily,days){
    ...
    currentUser.history.push(`Deposit ${amount} PKR for plan ${name}`);
...
function confirmWithdraw(){
    let amt=document.getElementById('withdrawAmount').value.trim();
    let type=document.getElementById('withdrawType').value;
    let acc=document.getElementById('withdrawAcc').value.trim();
    let file=document.getElementById('withdrawProof').files[0];
    if(!amt||!type||!acc||!file){showNotif('Fill all withdrawal fields'); return;}
    if(!currentUser.history) currentUser.history=[];
    currentUser.history.push(`Withdraw ${amt} PKR via ${type} (${acc})`);
