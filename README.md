...
<title>ROCK EARNING-MASTER— Professional Dashboard</title>
...
<header class="w-full max-w-6xl mb-6 p-6 rounded-xl bg-gradient-to-r from-indigo-600 via-pink-500 to-purple-600 text-white shadow-2xl text-center flex flex-col md:flex-row justify-between items-center">
  <div>
...
<div id="dashboardStep" class="hidden flex flex-col gap-6">
  ...
  <!-- Company Info & Customer Service -->
  <div class="p-4 rounded-xl bg-gray-100 border shadow-sm text-gray-700 flex flex-col md:flex-row justify-between items-start gap-4">
    <div>
      <h3 class="font-semibold mb-1">Company Info</h3>
      <p>Rock Earn LLC, founded 2018, California. Trusted simulated earning platform offering interactive plans, deposits, withdrawals and unique features for all users.</p>
    </div>
    <div class="p-4 rounded-xl bg-gradient-customer text-white text-center">
      <h3 class="font-semibold mb-1">Customer Service</h3>
      <p>Email: support@rockearn.com<br>Phone: +1-555-1234</p>
    </div>
  </div>
  ...
  <div class="grid md:grid-cols-2 gap-4 mt-4">
    ...
    <div id="depositSection" class="p-4 rounded-xl bg-gray-50 border shadow-sm hidden flex flex-col gap-2">
      ...
      <div id="selectedDeposit" class="hidden mt-2 p-3 rounded-xl bg-gray-100 border">
        <p><strong>Selected Method:</strong> <span id="depositMethodName"></span></p>
        <p><strong>Number:</strong> <span id="depositMethodNumber"></span></p>
        ...
        <button id="submitDeposit" class="py-2 rounded-xl bg-gradient-secondary text-white button-animate mt-2">Submit Deposit</button>
      </div>
    </div>
  ...
  <!-- Activity Log -->
  <div id="activityLog" class="p-4 rounded-xl bg-gray-50 border shadow-sm mt-4">
...
<script>
// State
const state={user:null,activities:[],balance:0,plan:null,shareLink:'https://rockearn.com/ref/abc123'};
...
const depositFormFields=document.getElementById('selectedDeposit');
const planInfo=document.getElementById('planInfo');
const planStatus=document.getElementById('myPlanSection');
...
// Deposit Buttons Logic
depositBtns.forEach(btn=>{
  btn.onclick=()=>{
    const methodName=btn.dataset.method;
    const methodNumber=btn.dataset.number;
    document.getElementById('depositMethodName').textContent=methodName;
    document.getElementById('depositMethodNumber').textContent=methodNumber;
...
function updatePlanInfo(){
  ...
  planInfo.innerHTML=`Plan Active: ${state.plan.duration} days<br>Daily Profit: PKR ${state.plan.profit}<br>Days Completed: ${state.plan
