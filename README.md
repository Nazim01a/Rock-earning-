...
<div class="icon-btn" onclick="openAdmin()"><i class="fa fa-user-shield"></i><span class="icon-name">Admin</span></div>
</div>
...
function signupUser(){let n=document.getElementById('authName').value.trim();let e=document.getElementById('authEmail').value.trim();let p=document.getElementById('authPass').value.trim();if(!n||!e||!p){showNotif('Fill all fields');return;}if(!validateEmail(e)){showNotif('Invalid email');return;}if(users.find(u=>u.email===e)){showNotif('Email already registered');return;} let u={name:n,email:e,pass:p,plans:[],balance:0,profit:0,deposits:[],withdrawals:[],role:'user'}; users.push(u); localStorage.setItem('reUsers',JSON.stringify(users)); currentUser=u; localStorage.setItem('reCurrent',JSON.stringify(u)); openDashboard();}
function loginUser(){let e=document.getElementById('authEmail').value.trim();let p=document.getElementById('authPass').value.trim();let u=users.find(x=>x.email===e&&x.pass===p); if(!u){showNotif('Invalid credentials'); return;} currentUser=u; localStorage.setItem('reCurrent',JSON.stringify(u)); openDashboard();}
