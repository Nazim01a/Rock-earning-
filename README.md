...
<style>
body{font-family:Arial,sans-serif;margin:0;padding:20px;background:#0d0d20;color:#fff;transition:background 0.3s}
button{padding:8px 12px;margin:4px;cursor:pointer;background:#00f7ef;color:#001;border:none;border-radius:6px;transition:0.3s}
button:hover{background:#00d5cc}
#sidebar{display:none;margin-top:20px}
.icon-btn{display:inline-block;width:100px;height:80px;text-align:center;border:1px solid #00f7ef;border-radius:8px;padding:8px;margin:4px;cursor:pointer;transition:0.3s}
.icon-btn:hover{background:#001f25}
#contentSection{margin-top:20px}
.plan-card{border:1px solid #00f7ef;padding:10px;margin:10px 0;border-radius:8px;background:rgba(0,247,239,0.05)}
#notif{position:fixed;top:10px;right:10px;background:#00f7ef;color:#001;padding:10px;border-radius:6px;display:none;font-weight:700}
</style>
</head>
...
const plans = [
  ...
  {name:'Ruby',amount:9000,daily:1750,days:50},
  {name:'Emerald',amount:11000,daily:2100,days:60},
  {name:'Sapphire',amount:13000,daily:2500,days:60},
  {name:'Titanium',amount:15000,daily:2900,days:70},
  {name:'Ultra',amount:17000,daily:3300,days:70},
  {name:'Mega',amount:20000,daily:4000,days:80},
  {name:'Ultimate',amount:25000,daily:5000,days:90}
];
...
function openDashboard(){
  ...
  showNotif('Welcome '+currentUser.name);
}
...
function openSection(type){
  ...
  if(type==='plans'){
    ...
    content.innerHTML=`<h3>Deposit Section</h3><p>Deposit Amount: Auto per plan</p>`;
  } else if(type==='withdraw'){
    content.innerHTML=`<h3>Withdraw Section</h3>`;
  } else if(type==='profit'){
    content.innerHTML=`<h3>Profit: ${currentUser.profit} PKR</h3>`;
  } else if(type==='history'){
    content.innerHTML=`<h3>History</h3>`;
    currentUser.plans.forEach(pl=>{
      content.innerHTML+=`<p>${pl.name} - ${pl.amount} PKR - Daily ${pl.daily} PKR - Days: ${pl.days}</p>`;
    });
...
function buyPlan(name,amount,daily,days){
  ...
  openSection('deposit');
  showNotif(`Plan ${name} bought. Deposit amount auto ready.`);
}
